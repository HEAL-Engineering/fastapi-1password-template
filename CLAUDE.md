...
Secrets are stored in 1Password vaults and generated to `.env` files:

```bash
task env:setup           # Install 1Password CLI, sign in
task env:generate        # Generate all .env files
task env:generate ENV=local  # Generate specific environment
```

**Vault naming:** `{VAULT_PREFIX}-LOCAL`, `{VAULT_PREFIX}-TEST`, `{VAULT_PREFIX}-PROD`

**Configure in:**
- `scripts/generate-env.sh` line 7: `VAULT_PREFIX="YOUR-PROJECT"`
- Each secret in 1Password = one environment variable (item title = var name)
